...
def closest_from_list(G, source, targets):
    # Get shortest path lengths from source to all reachable nodes
    lengths = nx.single_source_dijkstra_path_length(G, source)
    
    # Keep only the distances for nodes in the given list
    candidates = {node: lengths[node] for node in targets if node in lengths}
    
    if not candidates:
        return None, float("inf")  # no reachable target
    
    # Pick the closest one
    closest_node = min(candidates, key=candidates.get)
    return closest_node, candidates[closest_node]

def plot_matrix(matrix, cmap="viridis", show_colorbar=True, title=None):
...
def rrt_edge_exploration(grid, start, goal, walkable_cells, max_iter=5000):
    ...
    for iteration in range(max_iter):
        ...
        if target_cell in distances:
            ...
            current_pos = closest_from_list(visibility_graph,target_cell,visited)[0]
            path = nx.shortest_path(visibility_graph, current_pos, target_cell, weight='weight')
            ...
            for point in path:
                ...
                new_x, new_y = point[0] + 0.5, point[1] + 0.5
