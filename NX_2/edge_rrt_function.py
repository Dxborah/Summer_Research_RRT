...
def rrt_edge_exploration(grid, start, goal, walkable_cells, visibility_data, max_iter=5000):
    ...
    for iteration in range(max_iter):
        ...
        if goal_rechable == True or len(og_edge_seen) == 0:
            target_cell = goal
        else:
...
def run(seed, image, json_path, shelf_path, start, goal, max_iter=100, resize=(50, 50)):
    ...
    rrt_tree, goal_node_id, og_state = rrt_edge_exploration(grid, start, goal, walkable_cells,visibility_map, max_iter=max_iter)
    ...
    if goal_node_id is not None:
        ...
        draw_result_on_image(grid, rrt_tree, goal_node_id, visible_grid, "rrt_edge_explo_success.png")
    else:
        print(f"FAILED - No path found in {max_iter} iterations")
        print(f"Execution time: {execution_time:.2f} seconds")
        draw_result_on_image(grid, rrt_tree, None, visible_grid, "rrt_edge_explo_failed.png")
