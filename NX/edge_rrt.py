...
'''
def compute_edge_of_coverage(visible_grid, walkable_cells):
    """Compute the edge of coverage - frontier cells around visible area (blue cells)"""
    visible_cells = get_visible_cells(visible_grid)
    
    frontier_cells = set()
    # Look at 4-connected neighbors of visible cells
    directions = [(-1, 0), (1, 0), (0, -1), (0, 1)]
    
    for (x, y) in visible_cells:
        for dx, dy in directions:
            nx, ny = x + dx, y + dy
            # Check if neighbor is walkable but not visible
            if (nx, ny) in walkable_cells and (nx, ny) not in visible_cells:
